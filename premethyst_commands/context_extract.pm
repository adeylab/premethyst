package premethyst_commands::context_extract;

use premethyst_commands::general;
use Getopt::Std; %opt = ();
use Exporter "import";
@EXPORT = ("context_extract");
...
premethyst context-extract (options) [folder with cellcall files]
      or   context
...
>premethyst context-extract -O MyOut -C CH -N GCH -B myGCsites.bed -X HCH MyIn
   and
>premethyst context-extractl -O MyOut -C CG -N GCG -B myGCsites.bed -X HCG MyIn

Or for CAC methylation keeping regular CH files intact and adding to current folder:
>premethyst context-extract -C CH -N CAC -B myCACsites.bed.gz MyIn
